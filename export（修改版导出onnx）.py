...
import models
from models.experimental import attempt_load_0, End2End
from utils.activations import Hardswish, SiLU
from utils.general import set_logging_0, check_img_size_0
from utils.torch_utils import select_device_0
...
if __name__ == '__main__':
    ...
    # Load PyTorch model
    device = select_device_0(opt.device)
    model = attempt_load_0(opt.weights, map_location=device)  # load FP32 model
    labels = model.names
    
    # Checks
    gs = int(max(model.stride))  # grid size (max stride)
    opt.img_size = [check_img_size_0(x, gs) for x in opt.img_size]  # verify img_size are gs-multiples
